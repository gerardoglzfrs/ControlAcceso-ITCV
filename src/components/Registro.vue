<template>
  <v-app id="app">
    <v-container>
      ...
      <v-snackbar v-model="msjSuccess" multi-line>
        ¡Alumno registrado!
        <template v-slot:action="{ attrs }">
          <v-btn color="white" text v-bind="attrs" @click="msjSuccess = false" >
            <v-icon dark>mdi-close</v-icon>
          </v-btn>
        </template>
      </v-snackbar>
      <v-snackbar v-model="msjError" multi-line>
        ¡Número de control inexistente!
        <template v-slot:action="{ attrs }">
          <v-btn color="white" text v-bind="attrs" @click="msjSuccess = false" >
            <v-icon dark>mdi-close</v-icon>
          </v-btn>
      ...
      <v-form v-model="isValid" ref="formulario" v-on:submit.prevent>
      ...
      <v-row class="justify-center">
        <v-card class="mt-5">
          ...
          <v-data-table
            :headers="headers"
            :search="filtro"
            no-data-text="No hay entrada de alumnos"
            class="elevation-1"
            :loading="loading"
            loading-text="Cargando... Espere por favor"
            no-results-text="Número de control inexistente"
...
export default {
  ...
  data: () => ({
    ...
    idRules: [
      (value) => !!value || "Número de control requerido",
      (value) => {
        const number = /[^0-9]/g;
        return !number.test(value) || "Solo se permiten números";
      },
      (value) => (value || "").length === 8 || "Solo se permiten 8 digitos",
    ],
    filtro: "",
    loading: true,
    alumnos: [],
    headers: [
      ...
      { text: "Número de control", value: "controlNumber", sortable: false },
  ...
  methods: {
    ...
    async guardarRegistro() {
      ...
      try {
        ...
        `,
          variables: {
            controlNumber: parseInt(this.alumno.nc),
    ...
    async obtenerRegistro() {
      try {
        ...
        for (const val of data.getAllEntries) {
          ...
          minute = this.addZero(d.getMinutes());
